# 🎮 One-Day Hackathon: Build It. Level Up. Own the Stage!

Selamat datang di repository **One-Day Hackathon**! 🚀

Repository ini berisi materi pembelajaran untuk membuat game bertema lingkungan dengan berbagai tingkat kesulitan.

---

## 📋 Daftar Materi Workshop

| Level | Platform | Folder | Target Peserta |
|-------|----------|--------|----------------|
| 🟢 **Beginner** | Scratch | [📂 Scratch/](./Scratch/) | SD Kelas 4-6 / SMP |
| 🟡 **Intermediate** | Construct 2/3 | [📂 Construct 2/](./Construct%202/) | SMP / SMA |

---

## 🚀 Mulai Belajar

### 🟢 Level Beginner - Scratch

**Platform:** [Scratch](https://scratch.mit.edu/)  
**Cocok untuk:** Siswa yang baru mulai belajar pemrograman

#### 📚 Apa yang akan dipelajari?
- Logika dasar pemrograman (perulangan, kondisi)
- Membuat game Endless Runner (Siaga Banjir)
- Membuat game Clicker (Patroli Hutan Kalimantan)
- Membuat game Catching (Pilahan Sampah Nusantara)

#### 🔗 Mulai Tutorial
👉 **[Buka Tutorial Scratch](./Scratch/Readme.md)**

---

### 🟡 Level Intermediate - Construct 2/3

**Platform:** [Construct 2](https://www.scirra.com/construct2) / [Construct 3](https://www.construct.net/)  
**Cocok untuk:** Siswa yang sudah memahami konsep dasar game development

#### 📚 Apa yang akan dipelajari?
- Event system dan behaviors
- Physics dan collision detection
- Particle effects dan animations
- Game deployment dan export

#### 🔗 Mulai Tutorial
👉 **[Buka Tutorial Construct](./Construct%202/Readme.md)**

---

## 📁 Struktur Repository

```
📦 One-Day Hackathon
├── 📂 Scratch/              # Tutorial untuk Beginner
│   ├── Readme.md           # Dokumentasi lengkap
│   └── Assets/             # Sprite, background, sound (jika ada)
│
├── 📂 Construct 2/         # Tutorial untuk Intermediate
│   ├── Readme.md           # Dokumentasi lengkap
│   └── Assets/             # File project dan asset
│
└── 📂 App/                 # Aplikasi Timer & Leaderboard
    ├── index.html          # Admin panel
    └── display.html        # Display untuk proyektor
```

---

## 🎯 Tema Game: Edukasi Lingkungan

Semua game dalam hackathon ini mengangkat tema **pelestarian lingkungan**, meliputi:

| 🌊 Banjir | 🔥 Kebakaran Hutan | ♻️ Pengelolaan Sampah |
|-----------|-------------------|----------------------|
| Kesadaran bencana | Pencegahan kebakaran | Pemilahan sampah yang benar |

---

## 🛠️ Persiapan Sebelum Hackathon

### Untuk Level Beginner (Scratch)
1. ✅ Buka [Scratch Editor](https://scratch.mit.edu/projects/editor/)
2. ✅ Buat akun Scratch (opsional, untuk menyimpan project)
3. ✅ Siapkan mouse (disarankan)

### Untuk Level Intermediate (Construct)
1. ✅ Download [Construct 2](https://www.scirra.com/construct2/releases) atau gunakan [Construct 3 Online](https://editor.construct.net/)
2. ✅ Install software (untuk Construct 2)
3. ✅ Baca [dokumentasi dasar Construct](https://www.construct.net/en/make-games/manuals/construct-3)

---

## 🎨 Sumber Asset Gratis

| Sumber | Deskripsi |
|--------|-----------|
| [Kenney.nl](https://kenney.nl/) | Game assets gratis (sprite, tileset, UI) |
| [Flaticon.com](https://www.flaticon.com/) | Icon dan ilustrasi |
| [Game-Icons.net](https://Game-Icons.net/) | Icon game gratis |
| [OpenGameArt.org](https://opengameart.org/) | Asset game open source |
| [Freesound.org](https://freesound.org/) | Sound effect gratis |
| [Mixkit.co](https://Mixkit.co/) | Sound effect gratis |
| [Sfxr.me](https://Sfxr.me/) | Sound effect gratis |

---

## 📝 Lisensi

Materi hackathon ini dibuat untuk tujuan **edukasi** dan dapat digunakan secara bebas untuk pembelajaran.

---

**Build It. Level Up. Own the Stage! 🎮🔥**
